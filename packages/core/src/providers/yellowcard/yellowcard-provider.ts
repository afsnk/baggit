...
const CAPABILITIES: ProviderCapabilities = {
  ...
  supportedRegions: ["NG", "KE", "ZA"],
  supportedFiatCurrencies: ["USD", "NGN", "KES", "ZAR"],
  supportedCryptoAssets: ["BTC", "ETH", "USDC", "USDT"],
  supportedPaymentMethods: ["bank_transfer", "mobile_money", "mpesa", "debit_card"],
  kycRequired: true,
...
export class YellowCardProvider extends BaseProvider {
  ...
  async quote(request: QuoteRequest): Promise<ProviderQuote> {
    const rate = request.sourceAsset === "USDC" ? 1 : 0.95 + Math.random() * 0.08
    const feePercent = 0.8 + Math.random() * 1.2
    ...
    return {
      ...
      rate,
      fees: [
        { percentage: feePercent, flat: 0.25, currency: request.sourceAsset, description: "YellowCard fee" },
      ],
      totalFee: { percentage: feePercent, flat: 0.25, currency: request.sourceAsset },
      estimatedSettlementMs: 10_000 + Math.random() * 30_000,
  ...
  async initiate(request: TransactionRequest): Promise<Transaction> {
    return {
      ...
      region: request.region,
      status: "processing",
      providerTxId: `yc_${v4().slice(0, 12)}`,
      createdAt: new Date(),
      updatedAt: new Date(),
    }
  }
  ...
  async status(txId: string): Promise<SettlementStatus> {
    return Math.random() > 0.08 ? "completed" : "pending"
  }
}
