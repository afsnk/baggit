...
import { cn } from '#/lib/utils.ts' // Assuming you have a `cn` utility for merging Tailwind classes
 ...
 */
export const ReferralCTACard = React.forwardRef<
  HTMLDivElement,
  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          'w-full max-w-sm rounded-2xl bg-card p-0 text-card-foreground shadow-sm',
          className,
        )}
      >
        {/* Main Card Header */}
        <h2 className="text-xs lg:text-sm font-semibold text-muted-foreground">
          {title}
        </h2>

        {/* Main Amount Display */}
        <p className="text-sm lg:text-lg font-bold tracking-tight">
          Up to {currency}
        ...
        <div
          ...
          className="group mt-1 cursor-pointer rounded-xl bg-muted/50 p-3 transition-all duration-300 ease-in-out hover:bg-muted/80 hover:shadow-md"
        >
          ...
          <div className="mt-2 flex items-center">
            ...
            {moreCount && moreCount > 0 && (
              <div className="ml-[-12px] flex h-6 w-6 items-center justify-center rounded-full border-2 border-background bg-primary text-sm font-semibold text-primary-foreground">
                +{moreCount}
              </div>
            )}
          </div>
...
ReferralCTACard.displayName = 'ReferralCTACard'
