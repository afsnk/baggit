...
const response = await fetch(`/v1/checkout`, {
	method: "POST",
	headers: { // [!code focus]
		'Baggit-Public-Key': env.BAGGIT_PK, // [!code focus]
		'Baggit-Secret-Key': env.BAGGIT_SK, // [!code focus]
		'Content-Type': 'application/json' // [!code focus]
	}, // [!code focus]
	body: JSON.stringify({
		callbackUrl: 'https://webhook.site/#nnaosdvooosbdovbaobobvaosd',
		amount: 5000,
		currency: "NGN",
		type: "onetime"
	})
...
<Callout type="warn">
	DO NOT hardcode API Keys as plaintext in your code, this could result in severe security breach and potential loss of funds.
</Callout>
