...
export default function PayoutModal(props: IPayout) {
	...
	const [customAmount, setCustomAmount] = useState<number>(balance.data?.totalNgnBalance || 0)
        ...
        <FieldGroup className="gap-4 mt-4">
          <Field>
            ...
            <Select onValueChange={(value) => setBankCode(value)}>
              <SelectTrigger>
                <SelectValue placeholder={getBanks.isLoading ? "Loading banks..." : "Select bank"} />
              </SelectTrigger>
              <SelectContent>
          ...
          <Field orientation="vertical">
            <FieldLabel htmlFor="account_number" className="w-1/2">
              Account number
						...
						<Input id="account_number" placeholder="Enter account number" onChange={(e) => {
							if (e.target.value && e.target.value.length === 10 && bankCode) {
								lookupBank.mutate({
					...
					<Field>
						<InputGroup>
							<InputGroupInput
								...
								value={customAmount}
								onChange={(e) => {
									console.log(`Amount — Mutation`, {customAmount, amount: e.target.valueAsNumber})
							...
							<InputGroupAddon align="inline-start">
								<Select
									defaultValue='full'
									onValueChange={(value: 'full' | 'partial') => setWithdrawType(value)}
								>
