...
function RouteComponent() {
  ...
  const isOrchestra =
    routerState.location.pathname.includes('analytics') ||
          ...
          <Section className="p-4! sticky top-0 dark:bg-black bg-white z-50">
            <Container className="p-0! flex">
              <div className="capitalize flex items-center gap-2">
                ...
                {matched?.context.isUnderConstruction && (
                  <Tooltip>
                    <TooltipTrigger>
                      <Construction
                        className={cn('text-amber-400 animate-pulse size-4')}
                      />
                    </TooltipTrigger>
                    <TooltipContent>
                      <span className="max-w-xs text-wrap line-clamp-2 text-center">
                        This page is currently under active development, report
              ...
              <div className="flex-1 flex justify-end">
                {(routerState.location.pathname.slice(1).toLowerCase() ===
                  'analytics' ||
                  routerState.location.pathname.slice(1).toLowerCase() ==
                    'overview') && <Balance />}
              </div>
